...
def split(player,set)
	puts ""
	puts "[Split set: #{set}] => (1)hit (2)stand" 
	firstset=gets.chomp.to_i
    player<<$deck.pop if firstset==1 
	puts "---------------"
	puts ""
	$s1=cardsums(player) if set==1
	$s2=cardsums(player) if set==2
	if firstset==2
     puts "Ok, Set[1] stand on:#{$s1}" if set==1 || set==2
     puts "Ok, Set[2] stand on:#{$s2}" if set==2
     puts ""
     return
     end
    puts "You got .......... >> #{player[-1]}"
    puts ""
    puts ""
	  puts "Split set[#{set}] Total >>#{$s1}" if set==1 
    puts "Split set[#{set}] Total >>#{$s2}" if set==2
    return if $s1>=21 && set==1 
    return if set==2 && $s2>=21
    puts ""
    split(player,set)
end
 ...
 if choose==3
  $betamount*2<$money ? maxbet=$betamount*2: maxbet=$money
    ...
    break
 	...
 	if choose==4
    split([player[0]],1) #split deck1
 	  split([player[1]],2) #set==2 value here
    break
  end
  if cardsums(player)>21
    puts "You busted! you lost $#{$betamount}"
